// Code generated by `ubx sdk gen` from azure_sql_manageddatabasesensitivitylabels_sensitivity_label_2. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface ManageddatabasesensitivitylabelsSensitivityLabel2_Properties {
  clientClassificationSource?: string | Computed<string>;
  /** The column name. */
  columnName?: string | Computed<string>;
  /** The information type. */
  informationType?: string | Computed<string>;
  /** The information type ID. */
  informationTypeId?: string | Computed<string>;
  /** Is sensitivity recommendation disabled. Applicable for recommended sensitivity label only. Specifies whether the sensitivity recommendation on this column is disabled (dismissed) or not. */
  isDisabled?: boolean | Computed<boolean>;
  /** The label ID. */
  labelId?: string | Computed<string>;
  /** The label name. */
  labelName?: string | Computed<string>;
  rank?: string | Computed<string>;
  /** The schema name. */
  schemaName?: string | Computed<string>;
  /** The table name. */
  tableName?: string | Computed<string>;
}

const ManageddatabasesensitivitylabelsSensitivityLabel2_PropertiesFields: FieldMap = {
  clientClassificationSource: "client_classification_source",
  columnName: "column_name",
  informationType: "information_type",
  informationTypeId: "information_type_id",
  isDisabled: "is_disabled",
  labelId: "label_id",
  labelName: "label_name",
  rank: "rank",
  schemaName: "schema_name",
  tableName: "table_name",
};

export interface ManageddatabasesensitivitylabelsSensitivityLabel2Config {
  /** Properties of a sensitivity label. */
  properties?: ManageddatabasesensitivitylabelsSensitivityLabel2_Properties | Computed<ManageddatabasesensitivitylabelsSensitivityLabel2_Properties>;
}

export interface ManageddatabasesensitivitylabelsSensitivityLabel2Attrs {
  /** Resource that manages the sensitivity label. */
  managedBy: string;
  /** Properties of a sensitivity label. */
  properties: ManageddatabasesensitivitylabelsSensitivityLabel2_Properties;
}

export const ManageddatabasesensitivitylabelsSensitivityLabel2: ResourceBinding<ManageddatabasesensitivitylabelsSensitivityLabel2Config, ManageddatabasesensitivitylabelsSensitivityLabel2Attrs> = {
  wireType: "azure_sql_manageddatabasesensitivitylabels_sensitivity_label_2",
  fields: {
    properties: {
      wireName: "properties",
      kind: "object",
      fields: ManageddatabasesensitivitylabelsSensitivityLabel2_PropertiesFields,
    },
  },
};
