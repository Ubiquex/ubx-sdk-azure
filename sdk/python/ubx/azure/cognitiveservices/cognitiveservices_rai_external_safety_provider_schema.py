# Code generated by `ubx sdk gen` from azure_cognitiveservices_cognitiveservices_rai_external_safety_provider_schema. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class CognitiveservicesRaiExternalSafetyProviderSchema_Properties:
    # Creation time of the safety provider.
    created_at: Any = None
    # The Key Vault URI that contains the api key for safety provider urls.
    key_vault_uri: Any = None
    # Last modified time of the safety provider.
    last_modified_at: Any = None
    # The managed identity to access the Key Vault.
    managed_identity: Any = None
    # Safety provider mode sync/async.
    mode: Any = None
    # The unique identifier of the safety provider.
    provider_id: Any = None
    # Name of the safety provider.
    provider_name: Any = None
    # The name of the secret in Key Vault that contains the api key to access the webhook.
    secret_name: Any = None
    # Webhook URL for the safety provider.
    url: Any = None

_CognitiveservicesRaiExternalSafetyProviderSchema_PropertiesFields = {
    "created_at": ubx.FieldSpec(wire_name="created_at"),
    "key_vault_uri": ubx.FieldSpec(wire_name="key_vault_uri"),
    "last_modified_at": ubx.FieldSpec(wire_name="last_modified_at"),
    "managed_identity": ubx.FieldSpec(wire_name="managed_identity"),
    "mode": ubx.FieldSpec(wire_name="mode"),
    "provider_id": ubx.FieldSpec(wire_name="provider_id"),
    "provider_name": ubx.FieldSpec(wire_name="provider_name"),
    "secret_name": ubx.FieldSpec(wire_name="secret_name"),
    "url": ubx.FieldSpec(wire_name="url"),
}

@dataclasses.dataclass
class CognitiveservicesRaiExternalSafetyProviderSchemaConfig:
    # Resource Etag.
    etag: Any = None
    # RAI External SafetyProvider schema properties.
    properties: Any = None
    # Resource tags.
    tags: Any = None

@dataclasses.dataclass
class CognitiveservicesRaiExternalSafetyProviderSchemaAttrs:
    # Resource Etag.
    etag: Any = None
    # RAI External SafetyProvider schema properties.
    properties: Any = None
    # Resource tags.
    tags: Any = None

CognitiveservicesRaiExternalSafetyProviderSchema = ubx.ResourceBinding(
    wire_type="azure_cognitiveservices_cognitiveservices_rai_external_safety_provider_schema",
    fields={
        "etag": ubx.FieldSpec(wire_name="etag"),
        "properties": ubx.FieldSpec(
            wire_name="properties",
            kind="object",
            fields=_CognitiveservicesRaiExternalSafetyProviderSchema_PropertiesFields,
        ),
        "tags": ubx.FieldSpec(wire_name="tags"),
    },
)
