# Code generated by `ubx sdk gen` from azure_appconfiguration_appconfiguration_snapshot. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class AppconfigurationSnapshot_Properties_Filters:
    key: Any = None
    label: Any = None

@dataclasses.dataclass
class AppconfigurationSnapshot_Properties:
    # The composition type describes how the key-values within the snapshot are composed. The 'key' composition type ensures there are no two key-values containing the same key. The 'key_label' composition type ensures there are no two key-values containing the same key and label.
    composition_type: Any = None
    # The time that the snapshot was created.
    created: Any = None
    # A value representing the current state of the snapshot.
    etag: Any = None
    # The time that the snapshot will expire.
    expires: Any = None
    # A list of filters used to filter the key-values included in the snapshot.
    filters: Any = None
    # The amount of key-values in the snapshot.
    items_count: Any = None
    # The provisioning state of the snapshot.
    provisioning_state: Any = None
    # The amount of time, in seconds, that a snapshot will remain in the archived state before expiring. This property is only writable during the creation of a snapshot. If not specified, the default lifetime of key-value revisions will be used.
    retention_period: Any = None
    # The size in bytes of the snapshot.
    size: Any = None
    # The current status of the snapshot.
    status: Any = None
    # The tags of the snapshot. NOTE: These are data plane tags, not Azure Resource Manager (ARM) tags.
    tags: Any = None

_AppconfigurationSnapshot_Properties_FiltersFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "label": ubx.FieldSpec(wire_name="label"),
}

_AppconfigurationSnapshot_PropertiesFields = {
    "composition_type": ubx.FieldSpec(wire_name="composition_type"),
    "created": ubx.FieldSpec(wire_name="created"),
    "etag": ubx.FieldSpec(wire_name="etag"),
    "expires": ubx.FieldSpec(wire_name="expires"),
    "filters": ubx.FieldSpec(
        wire_name="filters",
        kind="list",
        fields=_AppconfigurationSnapshot_Properties_FiltersFields,
    ),
    "items_count": ubx.FieldSpec(wire_name="items_count"),
    "provisioning_state": ubx.FieldSpec(wire_name="provisioning_state"),
    "retention_period": ubx.FieldSpec(wire_name="retention_period"),
    "size": ubx.FieldSpec(wire_name="size"),
    "status": ubx.FieldSpec(wire_name="status"),
    "tags": ubx.FieldSpec(wire_name="tags"),
}

@dataclasses.dataclass
class AppconfigurationSnapshotConfig:
    # The resource ID.
    id: Any = None
    # The name of the snapshot.
    name: Any = None
    # All snapshot properties.
    properties: Any = None
    # The type of the resource.
    type: Any = None

@dataclasses.dataclass
class AppconfigurationSnapshotAttrs:
    # The resource ID.
    id: Any = None
    # The name of the snapshot.
    name: Any = None
    # All snapshot properties.
    properties: Any = None
    # The type of the resource.
    type: Any = None

AppconfigurationSnapshot = ubx.ResourceBinding(
    wire_type="azure_appconfiguration_appconfiguration_snapshot",
    fields={
        "id": ubx.FieldSpec(wire_name="id"),
        "name": ubx.FieldSpec(wire_name="name"),
        "properties": ubx.FieldSpec(
            wire_name="properties",
            kind="object",
            fields=_AppconfigurationSnapshot_PropertiesFields,
        ),
        "type": ubx.FieldSpec(wire_name="type"),
    },
)
