# Code generated by `ubx sdk gen` from azure_appconfiguration_appconfiguration_key_value. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class AppconfigurationKeyValue_Properties:
    # The content type of the key-value's value. Providing a proper content-type can enable transformations of values when they are retrieved by applications.
    content_type: Any = None
    # An ETag indicating the state of a key-value within a configuration store.
    e_tag: Any = None
    # The primary identifier of a key-value. The key is used in unison with the label to uniquely identify a key-value.
    key: Any = None
    # A value used to group key-values. The label is used in unison with the key to uniquely identify a key-value.
    label: Any = None
    # The last time a modifying operation was performed on the given key-value.
    last_modified: Any = None
    # A value indicating whether the key-value is locked. A locked key-value may not be modified until it is unlocked.
    locked: Any = None
    # A dictionary of tags that can help identify what a key-value may be applicable for.
    tags: Any = None
    # The value of the key-value.
    value: Any = None

_AppconfigurationKeyValue_PropertiesFields = {
    "content_type": ubx.FieldSpec(wire_name="content_type"),
    "e_tag": ubx.FieldSpec(wire_name="e_tag"),
    "key": ubx.FieldSpec(wire_name="key"),
    "label": ubx.FieldSpec(wire_name="label"),
    "last_modified": ubx.FieldSpec(wire_name="last_modified"),
    "locked": ubx.FieldSpec(wire_name="locked"),
    "tags": ubx.FieldSpec(wire_name="tags"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class AppconfigurationKeyValueConfig:
    # The resource ID.
    id: Any = None
    # The name of the resource.
    name: Any = None
    # All key-value properties.
    properties: Any = None
    # The type of the resource.
    type: Any = None

@dataclasses.dataclass
class AppconfigurationKeyValueAttrs:
    # The resource ID.
    id: Any = None
    # The name of the resource.
    name: Any = None
    # All key-value properties.
    properties: Any = None
    # The type of the resource.
    type: Any = None

AppconfigurationKeyValue = ubx.ResourceBinding(
    wire_type="azure_appconfiguration_appconfiguration_key_value",
    fields={
        "id": ubx.FieldSpec(wire_name="id"),
        "name": ubx.FieldSpec(wire_name="name"),
        "properties": ubx.FieldSpec(
            wire_name="properties",
            kind="object",
            fields=_AppconfigurationKeyValue_PropertiesFields,
        ),
        "type": ubx.FieldSpec(wire_name="type"),
    },
)
