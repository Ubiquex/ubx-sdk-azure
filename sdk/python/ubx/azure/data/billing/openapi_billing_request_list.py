# Code generated by `ubx sdk gen` from azure_billing_openapi_billing_request_list. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class OpenapiBillingRequestList_Value_Properties_CreatedBy:
    object_id: Any = None
    tenant_id: Any = None
    upn: Any = None

@dataclasses.dataclass
class OpenapiBillingRequestList_Value_Properties:
    additional_information: Any = None
    billing_account_display_name: Any = None
    billing_account_id: Any = None
    billing_account_name: Any = None
    billing_account_primary_billing_tenant_id: Any = None
    billing_profile_display_name: Any = None
    billing_profile_id: Any = None
    billing_profile_name: Any = None
    billing_scope: Any = None
    created_by: Any = None
    creation_date: Any = None
    customer_display_name: Any = None
    customer_id: Any = None
    customer_name: Any = None
    decision_reason: Any = None
    expiration_date: Any = None
    invoice_section_display_name: Any = None
    invoice_section_id: Any = None
    invoice_section_name: Any = None
    justification: Any = None
    last_updated_by: Any = None
    last_updated_date: Any = None
    provisioning_state: Any = None
    recipients: Any = None
    request_scope: Any = None
    reviewal_date: Any = None
    reviewed_by: Any = None
    status: Any = None
    subscription_display_name: Any = None
    subscription_id: Any = None
    subscription_name: Any = None
    type: Any = None

@dataclasses.dataclass
class OpenapiBillingRequestList_Value:
    properties: Any = None
    tags: Any = None

@dataclasses.dataclass
class OpenapiBillingRequestListConfig:
    billing_account_name: Any = None
    billing_profile_name: Any = None
    count: Any = None
    filter: Any = None
    order_by: Any = None
    search: Any = None
    skip: Any = None
    top: Any = None

@dataclasses.dataclass
class OpenapiBillingRequestListAttrs:
    billing_account_name: Any = None
    billing_profile_name: Any = None
    count: Any = None
    filter: Any = None
    # The link to the next page of items
    next_link: Any = None
    order_by: Any = None
    search: Any = None
    skip: Any = None
    top: Any = None
    # The BillingRequest items on this page
    value: Any = None

OpenapiBillingRequestList = ubx.DataSourceBinding(
    wire_type="azure_billing_openapi_billing_request_list",
    fields={
        "billing_account_name": ubx.FieldSpec(wire_name="billing_account_name"),
        "billing_profile_name": ubx.FieldSpec(wire_name="billing_profile_name"),
        "count": ubx.FieldSpec(wire_name="count"),
        "filter": ubx.FieldSpec(wire_name="filter"),
        "order_by": ubx.FieldSpec(wire_name="order_by"),
        "search": ubx.FieldSpec(wire_name="search"),
        "skip": ubx.FieldSpec(wire_name="skip"),
        "top": ubx.FieldSpec(wire_name="top"),
    },
)
