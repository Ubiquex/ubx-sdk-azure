# Code generated by `ubx sdk gen` from azure_automation_openapi_dsc_node_report_list. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class OpenapiDscNodeReportList_Value_Errors:
    error_code: Any = None
    error_details: Any = None
    error_message: Any = None
    error_source: Any = None
    locale: Any = None
    resource_id: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportList_Value_MetaConfiguration:
    action_after_reboot: Any = None
    allow_module_overwrite: Any = None
    certificate_id: Any = None
    configuration_mode: Any = None
    configuration_mode_frequency_mins: Any = None
    reboot_node_if_needed: Any = None
    refresh_frequency_mins: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportList_Value_Resources_DependsOn:
    resource_id: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportList_Value_Resources:
    depends_on: Any = None
    duration_in_seconds: Any = None
    error: Any = None
    module_name: Any = None
    module_version: Any = None
    resource_id: Any = None
    resource_name: Any = None
    source_info: Any = None
    start_date: Any = None
    status: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportList_Value:
    configuration_version: Any = None
    end_time: Any = None
    errors: Any = None
    host_name: Any = None
    i_pv4_addresses: Any = None
    i_pv6_addresses: Any = None
    id: Any = None
    last_modified_time: Any = None
    meta_configuration: Any = None
    number_of_resources: Any = None
    raw_errors: Any = None
    reboot_requested: Any = None
    refresh_mode: Any = None
    report_format_version: Any = None
    report_id: Any = None
    resources: Any = None
    start_time: Any = None
    status: Any = None
    type: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportListConfig:
    automation_account_name: Any = None
    node_id: Any = None

@dataclasses.dataclass
class OpenapiDscNodeReportListAttrs:
    automation_account_name: Any = None
    # The link to the next page of items
    next_link: Any = None
    node_id: Any = None
    # The DscNodeReport items on this page
    value: Any = None

OpenapiDscNodeReportList = ubx.DataSourceBinding(
    wire_type="azure_automation_openapi_dsc_node_report_list",
    fields={
        "automation_account_name": ubx.FieldSpec(wire_name="automation_account_name"),
        "node_id": ubx.FieldSpec(wire_name="node_id"),
    },
)
