# Code generated by `ubx sdk gen` from azure_recoveryservicessiterecovery_service_recovery_plan_collection. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties_CurrentScenario:
    job_id: Any = None
    scenario_name: Any = None
    start_time: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties_Groups_EndGroupActions_CustomDetails:
    instance_type: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties_Groups_EndGroupActions:
    action_name: Any = None
    custom_details: Any = None
    failover_directions: Any = None
    failover_types: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties_Groups_ReplicationProtectedItems:
    id: Any = None
    virtual_machine_id: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties_Groups:
    end_group_actions: Any = None
    group_type: Any = None
    replication_protected_items: Any = None
    start_group_actions: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value_Properties:
    allowed_operations: Any = None
    current_scenario: Any = None
    current_scenario_status: Any = None
    current_scenario_status_description: Any = None
    failover_deployment_model: Any = None
    friendly_name: Any = None
    groups: Any = None
    last_planned_failover_time: Any = None
    last_test_failover_time: Any = None
    last_unplanned_failover_time: Any = None
    primary_fabric_friendly_name: Any = None
    primary_fabric_id: Any = None
    provider_specific_details: Any = None
    recovery_fabric_friendly_name: Any = None
    recovery_fabric_id: Any = None
    replication_providers: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollection_Value:
    location: Any = None
    properties: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollectionConfig:
    resource_name: Any = None

@dataclasses.dataclass
class ServiceRecoveryPlanCollectionAttrs:
    # The link to the next page of items
    next_link: Any = None
    resource_name: Any = None
    # The RecoveryPlan items on this page
    value: Any = None

ServiceRecoveryPlanCollection = ubx.DataSourceBinding(
    wire_type="azure_recoveryservicessiterecovery_service_recovery_plan_collection",
    fields={
        "resource_name": ubx.FieldSpec(wire_name="resource_name"),
    },
)
