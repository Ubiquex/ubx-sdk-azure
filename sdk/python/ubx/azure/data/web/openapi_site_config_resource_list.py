# Code generated by `ubx sdk gen` from azure_web_openapi_site_config_resource_list. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_ApiDefinition:
    url: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_ApiManagementConfig:
    id: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AppSettings:
    name: Any = None
    value: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Actions_CustomAction:
    exe: Any = None
    parameters: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Actions:
    action_type: Any = None
    custom_action: Any = None
    min_process_execution_time: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Triggers_Requests:
    count: Any = None
    time_interval: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Triggers_SlowRequests:
    count: Any = None
    path: Any = None
    time_interval: Any = None
    time_taken: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Triggers_StatusCodes:
    count: Any = None
    path: Any = None
    status: Any = None
    sub_status: Any = None
    time_interval: Any = None
    win32_status: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Triggers_StatusCodesRange:
    count: Any = None
    path: Any = None
    status_codes: Any = None
    time_interval: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules_Triggers:
    private_bytes_in_kb: Any = None
    requests: Any = None
    slow_requests: Any = None
    slow_requests_with_path: Any = None
    status_codes: Any = None
    status_codes_range: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AutoHealRules:
    actions: Any = None
    triggers: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_AzureStorageAccounts:
    access_key: Any = None
    account_name: Any = None
    mount_path: Any = None
    protocol: Any = None
    share_name: Any = None
    state: Any = None
    type: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_ConnectionStrings:
    connection_string: Any = None
    name: Any = None
    type: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Cors:
    allowed_origins: Any = None
    support_credentials: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Experiments_RampUpRules:
    action_host_name: Any = None
    change_decision_callback_url: Any = None
    change_interval_in_minutes: Any = None
    change_step: Any = None
    max_reroute_percentage: Any = None
    min_reroute_percentage: Any = None
    name: Any = None
    reroute_percentage: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Experiments:
    ramp_up_rules: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_HandlerMappings:
    arguments: Any = None
    extension: Any = None
    script_processor: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_IpSecurityRestrictions:
    action: Any = None
    description: Any = None
    headers: Any = None
    ip_address: Any = None
    name: Any = None
    priority: Any = None
    subnet_mask: Any = None
    subnet_traffic_tag: Any = None
    tag: Any = None
    vnet_subnet_resource_id: Any = None
    vnet_traffic_tag: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Limits:
    max_disk_size_in_mb: Any = None
    max_memory_in_mb: Any = None
    max_percentage_cpu: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_MachineKey:
    decryption: Any = None
    decryption_key: Any = None
    validation: Any = None
    validation_key: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Push_Properties:
    dynamic_tags_json: Any = None
    is_push_enabled: Any = None
    tag_whitelist_json: Any = None
    tags_requiring_auth: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_Push:
    properties: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_VirtualApplications_VirtualDirectories:
    physical_path: Any = None
    virtual_path: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties_VirtualApplications:
    physical_path: Any = None
    preload_enabled: Any = None
    virtual_directories: Any = None
    virtual_path: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value_Properties:
    acr_use_managed_identity_creds: Any = None
    acr_user_managed_identity_id: Any = None
    always_on: Any = None
    api_definition: Any = None
    api_management_config: Any = None
    app_command_line: Any = None
    app_settings: Any = None
    auto_heal_enabled: Any = None
    auto_heal_rules: Any = None
    auto_swap_slot_name: Any = None
    azure_storage_accounts: Any = None
    connection_strings: Any = None
    cors: Any = None
    default_documents: Any = None
    detailed_error_logging_enabled: Any = None
    document_root: Any = None
    elastic_web_app_scale_limit: Any = None
    experiments: Any = None
    ftps_state: Any = None
    function_app_scale_limit: Any = None
    functions_runtime_scale_monitoring_enabled: Any = None
    handler_mappings: Any = None
    health_check_path: Any = None
    http20_enabled: Any = None
    http20_proxy_flag: Any = None
    http_logging_enabled: Any = None
    ip_security_restrictions: Any = None
    ip_security_restrictions_default_action: Any = None
    java_container: Any = None
    java_container_version: Any = None
    java_version: Any = None
    key_vault_reference_identity: Any = None
    limits: Any = None
    linux_fx_version: Any = None
    load_balancing: Any = None
    local_my_sql_enabled: Any = None
    logs_directory_size_limit: Any = None
    machine_key: Any = None
    managed_pipeline_mode: Any = None
    managed_service_identity_id: Any = None
    metadata: Any = None
    min_tls_cipher_suite: Any = None
    min_tls_version: Any = None
    minimum_elastic_instance_count: Any = None
    net_framework_version: Any = None
    node_version: Any = None
    number_of_workers: Any = None
    php_version: Any = None
    power_shell_version: Any = None
    pre_warmed_instance_count: Any = None
    public_network_access: Any = None
    publishing_username: Any = None
    push: Any = None
    python_version: Any = None
    remote_debugging_enabled: Any = None
    remote_debugging_version: Any = None
    request_tracing_enabled: Any = None
    request_tracing_expiration_time: Any = None
    scm_ip_security_restrictions: Any = None
    scm_ip_security_restrictions_default_action: Any = None
    scm_ip_security_restrictions_use_main: Any = None
    scm_min_tls_version: Any = None
    scm_type: Any = None
    tracing_options: Any = None
    use32_bit_worker_process: Any = None
    virtual_applications: Any = None
    vnet_name: Any = None
    vnet_private_ports_count: Any = None
    vnet_route_all_enabled: Any = None
    web_sockets_enabled: Any = None
    website_time_zone: Any = None
    windows_fx_version: Any = None
    x_managed_service_identity_id: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceList_Value:
    kind: Any = None
    properties: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceListConfig:
    name: Any = None

@dataclasses.dataclass
class OpenapiSiteConfigResourceListAttrs:
    name: Any = None
    # The link to the next page of items
    next_link: Any = None
    # The SiteConfigResource items on this page
    value: Any = None

OpenapiSiteConfigResourceList = ubx.DataSourceBinding(
    wire_type="azure_web_openapi_site_config_resource_list",
    fields={
        "name": ubx.FieldSpec(wire_name="name"),
    },
)
