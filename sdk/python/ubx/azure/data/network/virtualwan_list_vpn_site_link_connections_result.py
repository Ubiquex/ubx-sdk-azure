# Code generated by `ubx sdk gen` from azure_network_virtualwan_list_vpn_site_link_connections_result. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResult_Value_Properties_EgressNatRules:
    id: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResult_Value_Properties_IpsecPolicies:
    dh_group: Any = None
    ike_encryption: Any = None
    ike_integrity: Any = None
    ipsec_encryption: Any = None
    ipsec_integrity: Any = None
    pfs_group: Any = None
    sa_data_size_kilobytes: Any = None
    sa_life_time_seconds: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResult_Value_Properties_VpnGatewayCustomBgpAddresses:
    custom_bgp_ip_address: Any = None
    ip_configuration_id: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResult_Value_Properties:
    connection_bandwidth: Any = None
    connection_status: Any = None
    dpd_timeout_seconds: Any = None
    egress_bytes_transferred: Any = None
    egress_nat_rules: Any = None
    enable_bgp: Any = None
    enable_rate_limiting: Any = None
    ingress_bytes_transferred: Any = None
    ingress_nat_rules: Any = None
    ipsec_policies: Any = None
    provisioning_state: Any = None
    routing_weight: Any = None
    shared_key: Any = None
    use_local_azure_ip_address: Any = None
    use_policy_based_traffic_selectors: Any = None
    vpn_connection_protocol_type: Any = None
    vpn_gateway_custom_bgp_addresses: Any = None
    vpn_link_connection_mode: Any = None
    vpn_site_link: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResult_Value:
    etag: Any = None
    properties: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResultConfig:
    connection_name: Any = None
    gateway_name: Any = None

@dataclasses.dataclass
class VirtualwanListVpnSiteLinkConnectionsResultAttrs:
    connection_name: Any = None
    gateway_name: Any = None
    # The link to the next page of items
    next_link: Any = None
    # The VpnSiteLinkConnection items on this page
    value: Any = None

VirtualwanListVpnSiteLinkConnectionsResult = ubx.DataSourceBinding(
    wire_type="azure_network_virtualwan_list_vpn_site_link_connections_result",
    fields={
        "connection_name": ubx.FieldSpec(wire_name="connection_name"),
        "gateway_name": ubx.FieldSpec(wire_name="gateway_name"),
    },
)
