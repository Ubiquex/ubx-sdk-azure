// Code generated by `ubx sdk gen` from azure_billing_openapi_product_list_result. DO NOT EDIT.
package billing

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type OpenapiProductListResult_Value_Properties_LastCharge struct {
	Currency any
	Value any
}

type OpenapiProductListResult_Value_Properties_Reseller struct {
	Description any
	ResellerId any
}

type OpenapiProductListResult_Value_Properties struct {
	AutoRenew any
	AvailabilityId any
	BillingFrequency any
	BillingProfileDisplayName any
	BillingProfileId any
	CustomerDisplayName any
	CustomerId any
	DisplayName any
	EndDate any
	InvoiceSectionDisplayName any
	InvoiceSectionId any
	LastCharge any
	LastChargeDate any
	ProductType any
	ProductTypeId any
	PurchaseDate any
	Quantity any
	Reseller any
	SkuDescription any
	SkuId any
	Status any
	TenantId any
}

type OpenapiProductListResult_Value struct {
	Properties any
	Tags any
}

type OpenapiProductListResultConfig struct {
	BillingAccountName any
	BillingProfileName any
	Count any
	Filter any
	InvoiceSectionName any
	OrderBy any
	Search any
	Skip any
	Top any
}

type OpenapiProductListResultAttrs struct {
	BillingAccountName any
	BillingProfileName any
	Count any
	Filter any
	InvoiceSectionName any
	// The link to the next page of items
	NextLink any
	OrderBy any
	Search any
	Skip any
	Top any
	// The Product items on this page
	Value any
}

var OpenapiProductListResult = ubx.DataSourceBinding{
	WireType: "azure_billing_openapi_product_list_result",
	Fields: ubx.FieldMap{
		"BillingAccountName": ubx.FieldSpec{WireName: "billing_account_name"},
		"BillingProfileName": ubx.FieldSpec{WireName: "billing_profile_name"},
		"Count": ubx.FieldSpec{WireName: "count"},
		"Filter": ubx.FieldSpec{WireName: "filter"},
		"InvoiceSectionName": ubx.FieldSpec{WireName: "invoice_section_name"},
		"OrderBy": ubx.FieldSpec{WireName: "order_by"},
		"Search": ubx.FieldSpec{WireName: "search"},
		"Skip": ubx.FieldSpec{WireName: "skip"},
		"Top": ubx.FieldSpec{WireName: "top"},
	},
}
