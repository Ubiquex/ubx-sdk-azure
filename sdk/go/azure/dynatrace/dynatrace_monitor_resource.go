// Code generated by `ubx sdk gen` from azure_dynatrace_dynatrace_monitor_resource. DO NOT EDIT.
package dynatrace

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DynatraceMonitorResource_Identity_UserAssignedIdentities struct {
	ClientId any
	PrincipalId any
}

type DynatraceMonitorResource_Identity struct {
	// The active directory identifier of this principal.
	PrincipalId any
	// The Active Directory tenant id of the principal.
	TenantId any
	// The kind of managed identity assigned to this resource.
	Type any
	// The identities assigned to this resource by the user.
	UserAssignedIdentities any
}

type DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_AccountInfo struct {
	// Account Id of the account this environment is linked to
	AccountId any
	// Name of the customer account / company
	CompanyName any
	// Region in which the account is created
	RegionId any
}

type DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_EnvironmentInfo struct {
	// Id of the environment created
	EnvironmentId any
	// Ingestion key with secret and read-only attributes for all API versions.
	IngestionKey any
	// Landing URL for Dynatrace environment
	LandingUrl any
	// Ingestion endpoint used for sending logs
	LogsIngestionEndpoint any
}

type DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_SingleSignOnProperties struct {
	// array of Aad(azure active directory) domains
	AadDomains any
	// Version of the Dynatrace agent installed on the VM.
	EnterpriseAppId any
	// Provisioning state of the monitoring resource
	ProvisioningState any
	// Various states of the SSO resource
	SingleSignOnState any
	// The login URL specific to this Dynatrace Environment
	SingleSignOnUrl any
}

type DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties struct {
	// Dynatrace Account Information
	AccountInfo any
	// Dynatrace Environment Information
	EnvironmentInfo any
	// The details of a Dynatrace single sign-on.
	SingleSignOnProperties any
	// User id
	UserId any
}

type DynatraceMonitorResource_Properties_PlanData struct {
	// different billing cycles like MONTHLY/WEEKLY. this could be enum
	BillingCycle any
	// date when plan was applied
	EffectiveDate any
	// plan id as published by Dynatrace
	PlanDetails any
	// different usage type like PAYG/COMMITTED. this could be enum
	UsageType any
}

type DynatraceMonitorResource_Properties_UserInfo struct {
	// Country of the user
	Country any
	// Email of the user used by Dynatrace for contacting them if needed
	EmailAddress any
	// First Name of the user
	FirstName any
	// Last Name of the user
	LastName any
	// Phone number of the user used by Dynatrace for contacting them if needed
	PhoneNumber any
}

type DynatraceMonitorResource_Properties struct {
	// Properties of the Dynatrace environment.
	DynatraceEnvironmentProperties any
	// Liftr resource category
	LiftrResourceCategory any
	// The priority of the resource.
	LiftrResourcePreference any
	// Marketplace resource autorenew flag
	MarketplaceSaasAutoRenew any
	// Flag specifying the Marketplace Subscription Status of the resource. If payment is not made in time, the resource will go in Suspended state.
	MarketplaceSubscriptionStatus any
	// Flag specifying if the resource monitoring is enabled or disabled.
	MonitoringStatus any
	// Billing plan information.
	PlanData any
	// Provisioning state of the monitoring resource
	ProvisioningState any
	// User info.
	UserInfo any
}

var DynatraceMonitorResource_Identity_UserAssignedIdentitiesFields = ubx.FieldMap{
		"ClientId": ubx.FieldSpec{WireName: "client_id"},
		"PrincipalId": ubx.FieldSpec{WireName: "principal_id"},
	}

var DynatraceMonitorResource_IdentityFields = ubx.FieldMap{
		"PrincipalId": ubx.FieldSpec{WireName: "principal_id"},
		"TenantId": ubx.FieldSpec{WireName: "tenant_id"},
		"Type": ubx.FieldSpec{WireName: "type"},
		"UserAssignedIdentities": ubx.FieldSpec{
			WireName: "user_assigned_identities",
			Kind: "map",
			Fields: DynatraceMonitorResource_Identity_UserAssignedIdentitiesFields,
		},
	}

var DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_AccountInfoFields = ubx.FieldMap{
		"AccountId": ubx.FieldSpec{WireName: "account_id"},
		"CompanyName": ubx.FieldSpec{WireName: "company_name"},
		"RegionId": ubx.FieldSpec{WireName: "region_id"},
	}

var DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_EnvironmentInfoFields = ubx.FieldMap{
		"EnvironmentId": ubx.FieldSpec{WireName: "environment_id"},
		"IngestionKey": ubx.FieldSpec{WireName: "ingestion_key"},
		"LandingUrl": ubx.FieldSpec{WireName: "landing_url"},
		"LogsIngestionEndpoint": ubx.FieldSpec{WireName: "logs_ingestion_endpoint"},
	}

var DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_SingleSignOnPropertiesFields = ubx.FieldMap{
		"AadDomains": ubx.FieldSpec{WireName: "aad_domains"},
		"EnterpriseAppId": ubx.FieldSpec{WireName: "enterprise_app_id"},
		"ProvisioningState": ubx.FieldSpec{WireName: "provisioning_state"},
		"SingleSignOnState": ubx.FieldSpec{WireName: "single_sign_on_state"},
		"SingleSignOnUrl": ubx.FieldSpec{WireName: "single_sign_on_url"},
	}

var DynatraceMonitorResource_Properties_DynatraceEnvironmentPropertiesFields = ubx.FieldMap{
		"AccountInfo": ubx.FieldSpec{
			WireName: "account_info",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_AccountInfoFields,
		},
		"EnvironmentInfo": ubx.FieldSpec{
			WireName: "environment_info",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_EnvironmentInfoFields,
		},
		"SingleSignOnProperties": ubx.FieldSpec{
			WireName: "single_sign_on_properties",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_DynatraceEnvironmentProperties_SingleSignOnPropertiesFields,
		},
		"UserId": ubx.FieldSpec{WireName: "user_id"},
	}

var DynatraceMonitorResource_Properties_PlanDataFields = ubx.FieldMap{
		"BillingCycle": ubx.FieldSpec{WireName: "billing_cycle"},
		"EffectiveDate": ubx.FieldSpec{WireName: "effective_date"},
		"PlanDetails": ubx.FieldSpec{WireName: "plan_details"},
		"UsageType": ubx.FieldSpec{WireName: "usage_type"},
	}

var DynatraceMonitorResource_Properties_UserInfoFields = ubx.FieldMap{
		"Country": ubx.FieldSpec{WireName: "country"},
		"EmailAddress": ubx.FieldSpec{WireName: "email_address"},
		"FirstName": ubx.FieldSpec{WireName: "first_name"},
		"LastName": ubx.FieldSpec{WireName: "last_name"},
		"PhoneNumber": ubx.FieldSpec{WireName: "phone_number"},
	}

var DynatraceMonitorResource_PropertiesFields = ubx.FieldMap{
		"DynatraceEnvironmentProperties": ubx.FieldSpec{
			WireName: "dynatrace_environment_properties",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_DynatraceEnvironmentPropertiesFields,
		},
		"LiftrResourceCategory": ubx.FieldSpec{WireName: "liftr_resource_category"},
		"LiftrResourcePreference": ubx.FieldSpec{WireName: "liftr_resource_preference"},
		"MarketplaceSaasAutoRenew": ubx.FieldSpec{WireName: "marketplace_saas_auto_renew"},
		"MarketplaceSubscriptionStatus": ubx.FieldSpec{WireName: "marketplace_subscription_status"},
		"MonitoringStatus": ubx.FieldSpec{WireName: "monitoring_status"},
		"PlanData": ubx.FieldSpec{
			WireName: "plan_data",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_PlanDataFields,
		},
		"ProvisioningState": ubx.FieldSpec{WireName: "provisioning_state"},
		"UserInfo": ubx.FieldSpec{
			WireName: "user_info",
			Kind: "object",
			Fields: DynatraceMonitorResource_Properties_UserInfoFields,
		},
	}

type DynatraceMonitorResourceConfig struct {
	// The properties of the managed service identities assigned to this resource.
	Identity any
	// Properties specific to the monitor resource.
	Properties any
}

type DynatraceMonitorResourceAttrs struct {
	// The properties of the managed service identities assigned to this resource.
	Identity any
	// Properties specific to the monitor resource.
	Properties any
}

var DynatraceMonitorResource = ubx.ResourceBinding{
	WireType: "azure_dynatrace_dynatrace_monitor_resource",
	Fields: ubx.FieldMap{
		"Identity": ubx.FieldSpec{
			WireName: "identity",
			Kind: "object",
			Fields: DynatraceMonitorResource_IdentityFields,
		},
		"Properties": ubx.FieldSpec{
			WireName: "properties",
			Kind: "object",
			Fields: DynatraceMonitorResource_PropertiesFields,
		},
	},
}
