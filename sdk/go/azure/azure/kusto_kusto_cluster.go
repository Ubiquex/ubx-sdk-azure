// Code generated by `ubx sdk gen` from azure_azure_kusto_kusto_cluster. DO NOT EDIT.
package azure

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type KustoKustoCluster_Identity_UserAssignedIdentities struct {
	ClientId any
	PrincipalId any
}

type KustoKustoCluster_Identity struct {
	// The principal ID of resource identity.
	PrincipalId any
	// The tenant ID of resource.
	TenantId any
	// The type of managed identity used. The type 'SystemAssigned, UserAssigned' includes both an implicitly created identity and a set of user-assigned identities. The type 'None' will remove all identities.
	Type any
	// The list of user identities associated with the Kusto cluster. The user identity dictionary key references will be ARM resource ids in the form: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
	UserAssignedIdentities any
}

type KustoKustoCluster_Properties_AcceptedAudiences struct {
	Value any
}

type KustoKustoCluster_Properties_CalloutPolicies struct {
	CalloutId any
	CalloutType any
	CalloutUriRegex any
	OutboundAccess any
}

type KustoKustoCluster_Properties_KeyVaultProperties struct {
	// The application (client) ID of the multi-tenant Microsoft Entra application. Used for cross-tenant customer-managed key scenarios where the encryption key is stored in a different tenant than the cluster. The application must be configured with the user-assigned managed identity as a federated identity credential.
	FederatedIdentityClientId any
	// The name of the key vault key.
	KeyName any
	// The Uri of the key vault.
	KeyVaultUri any
	// The version of the key vault key.
	KeyVersion any
	// The user assigned identity (ARM resource id) that has access to the key. The identity must have 'Get', 'Wrap Key', and 'Unwrap Key' permissions on the Key Vault key, or be assigned the 'Key Vault Crypto Service Encryption User' role.
	UserIdentity any
}

type KustoKustoCluster_Properties_LanguageExtensions_Value struct {
	LanguageExtensionCustomImageName any
	LanguageExtensionImageName any
	LanguageExtensionName any
}

type KustoKustoCluster_Properties_LanguageExtensions struct {
	// The link to the next page of resources.
	NextLink any
	// The list of language extensions.
	Value any
}

type KustoKustoCluster_Properties_MigrationCluster struct {
	// The public data ingestion URL of the cluster.
	DataIngestionUri any
	// The resource ID of the cluster.
	Id any
	// The role of the cluster in the migration process.
	Role any
	// The public URL of the cluster.
	Uri any
}

type KustoKustoCluster_Properties_OptimizedAutoscale struct {
	// A boolean value that indicate if the optimized autoscale feature is enabled or not.
	IsEnabled any
	// Maximum allowed instances count.
	Maximum any
	// Minimum allowed instances count.
	Minimum any
	// The version of the template defined, for instance 1.
	Version any
}

type KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateEndpoint struct {
	Id any
}

type KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateLinkServiceConnectionState struct {
	ActionsRequired any
	Description any
	Status any
}

type KustoKustoCluster_Properties_PrivateEndpointConnections_Properties struct {
	GroupId any
	PrivateEndpoint any
	PrivateLinkServiceConnectionState any
	ProvisioningState any
}

type KustoKustoCluster_Properties_PrivateEndpointConnections struct {
	Properties any
}

type KustoKustoCluster_Properties_VirtualNetworkConfiguration struct {
	// Data management's service public IP address resource id.
	DataManagementPublicIpId any
	// Engine service's public IP address resource id.
	EnginePublicIpId any
	// When enabled, the cluster is deployed into the configured subnet, when disabled it will be removed from the subnet.
	State any
	// The subnet resource id.
	SubnetId any
}

type KustoKustoCluster_Properties struct {
	// The cluster's accepted audiences.
	AcceptedAudiences any
	// List of allowed FQDNs(Fully Qualified Domain Name) for egress from Cluster.
	AllowedFqdnList any
	// The list of ips in the format of CIDR allowed to connect to the cluster.
	AllowedIpRangeList any
	// List of callout policies for egress from Cluster.
	CalloutPolicies any
	// The cluster data ingestion URI.
	DataIngestionUri any
	// A boolean value that indicates if the cluster could be automatically stopped (due to lack of data or no activity for many days).
	EnableAutoStop any
	// A boolean value that indicates if the cluster's disks are encrypted.
	EnableDiskEncryption any
	// A boolean value that indicates if double encryption is enabled.
	EnableDoubleEncryption any
	// A boolean value that indicates if the purge operations are enabled.
	EnablePurge any
	// A boolean value that indicates if the streaming ingest is enabled.
	EnableStreamingIngest any
	// The engine type
	EngineType any
	// Properties of the key vault.
	KeyVaultProperties any
	// The list of language extension objects.
	LanguageExtensions any
	// Represents a properties of a cluster that is part of a migration.
	MigrationCluster any
	// A class that contains the optimized auto scale definition.
	OptimizedAutoscale any
	// A list of private endpoint connections.
	PrivateEndpointConnections any
	// The provisioned state of the resource.
	ProvisioningState any
	// Indicates what public IP type to create - IPv4 (default), or DualStack (both IPv4 and IPv6)
	PublicIptype any
	// Public network access to the cluster is enabled by default. When disabled, only private endpoint connection to the cluster is allowed
	PublicNetworkAccess any
	// Whether or not to restrict outbound network access. Value is optional but if passed in, must be 'Enabled' or 'Disabled'
	RestrictOutboundNetworkAccess any
	// The state of the resource.
	State any
	// The reason for the cluster's current state.
	StateReason any
	// The cluster's external tenants.
	TrustedExternalTenants any
	// The cluster URI.
	Uri any
	// Virtual Cluster graduation properties
	VirtualClusterGraduationProperties any
	// A class that contains virtual network definition.
	VirtualNetworkConfiguration any
	// Indicates whether the cluster is zonal or non-zonal.
	ZoneStatus any
}

type KustoKustoCluster_Sku struct {
	// The number of instances of the cluster.
	Capacity any
	// SKU name.
	Name any
	// SKU tier.
	Tier any
}

var KustoKustoCluster_Identity_UserAssignedIdentitiesFields = ubx.FieldMap{
		"ClientId": ubx.FieldSpec{WireName: "client_id"},
		"PrincipalId": ubx.FieldSpec{WireName: "principal_id"},
	}

var KustoKustoCluster_IdentityFields = ubx.FieldMap{
		"PrincipalId": ubx.FieldSpec{WireName: "principal_id"},
		"TenantId": ubx.FieldSpec{WireName: "tenant_id"},
		"Type": ubx.FieldSpec{WireName: "type"},
		"UserAssignedIdentities": ubx.FieldSpec{
			WireName: "user_assigned_identities",
			Kind: "map",
			Fields: KustoKustoCluster_Identity_UserAssignedIdentitiesFields,
		},
	}

var KustoKustoCluster_Properties_AcceptedAudiencesFields = ubx.FieldMap{
		"Value": ubx.FieldSpec{WireName: "value"},
	}

var KustoKustoCluster_Properties_CalloutPoliciesFields = ubx.FieldMap{
		"CalloutId": ubx.FieldSpec{WireName: "callout_id"},
		"CalloutType": ubx.FieldSpec{WireName: "callout_type"},
		"CalloutUriRegex": ubx.FieldSpec{WireName: "callout_uri_regex"},
		"OutboundAccess": ubx.FieldSpec{WireName: "outbound_access"},
	}

var KustoKustoCluster_Properties_KeyVaultPropertiesFields = ubx.FieldMap{
		"FederatedIdentityClientId": ubx.FieldSpec{WireName: "federated_identity_client_id"},
		"KeyName": ubx.FieldSpec{WireName: "key_name"},
		"KeyVaultUri": ubx.FieldSpec{WireName: "key_vault_uri"},
		"KeyVersion": ubx.FieldSpec{WireName: "key_version"},
		"UserIdentity": ubx.FieldSpec{WireName: "user_identity"},
	}

var KustoKustoCluster_Properties_LanguageExtensions_ValueFields = ubx.FieldMap{
		"LanguageExtensionCustomImageName": ubx.FieldSpec{WireName: "language_extension_custom_image_name"},
		"LanguageExtensionImageName": ubx.FieldSpec{WireName: "language_extension_image_name"},
		"LanguageExtensionName": ubx.FieldSpec{WireName: "language_extension_name"},
	}

var KustoKustoCluster_Properties_LanguageExtensionsFields = ubx.FieldMap{
		"NextLink": ubx.FieldSpec{WireName: "next_link"},
		"Value": ubx.FieldSpec{
			WireName: "value",
			Kind: "list",
			Fields: KustoKustoCluster_Properties_LanguageExtensions_ValueFields,
		},
	}

var KustoKustoCluster_Properties_MigrationClusterFields = ubx.FieldMap{
		"DataIngestionUri": ubx.FieldSpec{WireName: "data_ingestion_uri"},
		"Id": ubx.FieldSpec{WireName: "id"},
		"Role": ubx.FieldSpec{WireName: "role"},
		"Uri": ubx.FieldSpec{WireName: "uri"},
	}

var KustoKustoCluster_Properties_OptimizedAutoscaleFields = ubx.FieldMap{
		"IsEnabled": ubx.FieldSpec{WireName: "is_enabled"},
		"Maximum": ubx.FieldSpec{WireName: "maximum"},
		"Minimum": ubx.FieldSpec{WireName: "minimum"},
		"Version": ubx.FieldSpec{WireName: "version"},
	}

var KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateEndpointFields = ubx.FieldMap{
		"Id": ubx.FieldSpec{WireName: "id"},
	}

var KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateLinkServiceConnectionStateFields = ubx.FieldMap{
		"ActionsRequired": ubx.FieldSpec{WireName: "actions_required"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"Status": ubx.FieldSpec{WireName: "status"},
	}

var KustoKustoCluster_Properties_PrivateEndpointConnections_PropertiesFields = ubx.FieldMap{
		"GroupId": ubx.FieldSpec{WireName: "group_id"},
		"PrivateEndpoint": ubx.FieldSpec{
			WireName: "private_endpoint",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateEndpointFields,
		},
		"PrivateLinkServiceConnectionState": ubx.FieldSpec{
			WireName: "private_link_service_connection_state",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_PrivateEndpointConnections_Properties_PrivateLinkServiceConnectionStateFields,
		},
		"ProvisioningState": ubx.FieldSpec{WireName: "provisioning_state"},
	}

var KustoKustoCluster_Properties_PrivateEndpointConnectionsFields = ubx.FieldMap{
		"Properties": ubx.FieldSpec{
			WireName: "properties",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_PrivateEndpointConnections_PropertiesFields,
		},
	}

var KustoKustoCluster_Properties_VirtualNetworkConfigurationFields = ubx.FieldMap{
		"DataManagementPublicIpId": ubx.FieldSpec{WireName: "data_management_public_ip_id"},
		"EnginePublicIpId": ubx.FieldSpec{WireName: "engine_public_ip_id"},
		"State": ubx.FieldSpec{WireName: "state"},
		"SubnetId": ubx.FieldSpec{WireName: "subnet_id"},
	}

var KustoKustoCluster_PropertiesFields = ubx.FieldMap{
		"AcceptedAudiences": ubx.FieldSpec{
			WireName: "accepted_audiences",
			Kind: "list",
			Fields: KustoKustoCluster_Properties_AcceptedAudiencesFields,
		},
		"AllowedFqdnList": ubx.FieldSpec{WireName: "allowed_fqdn_list"},
		"AllowedIpRangeList": ubx.FieldSpec{WireName: "allowed_ip_range_list"},
		"CalloutPolicies": ubx.FieldSpec{
			WireName: "callout_policies",
			Kind: "list",
			Fields: KustoKustoCluster_Properties_CalloutPoliciesFields,
		},
		"DataIngestionUri": ubx.FieldSpec{WireName: "data_ingestion_uri"},
		"EnableAutoStop": ubx.FieldSpec{WireName: "enable_auto_stop"},
		"EnableDiskEncryption": ubx.FieldSpec{WireName: "enable_disk_encryption"},
		"EnableDoubleEncryption": ubx.FieldSpec{WireName: "enable_double_encryption"},
		"EnablePurge": ubx.FieldSpec{WireName: "enable_purge"},
		"EnableStreamingIngest": ubx.FieldSpec{WireName: "enable_streaming_ingest"},
		"EngineType": ubx.FieldSpec{WireName: "engine_type"},
		"KeyVaultProperties": ubx.FieldSpec{
			WireName: "key_vault_properties",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_KeyVaultPropertiesFields,
		},
		"LanguageExtensions": ubx.FieldSpec{
			WireName: "language_extensions",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_LanguageExtensionsFields,
		},
		"MigrationCluster": ubx.FieldSpec{
			WireName: "migration_cluster",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_MigrationClusterFields,
		},
		"OptimizedAutoscale": ubx.FieldSpec{
			WireName: "optimized_autoscale",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_OptimizedAutoscaleFields,
		},
		"PrivateEndpointConnections": ubx.FieldSpec{
			WireName: "private_endpoint_connections",
			Kind: "list",
			Fields: KustoKustoCluster_Properties_PrivateEndpointConnectionsFields,
		},
		"ProvisioningState": ubx.FieldSpec{WireName: "provisioning_state"},
		"PublicIptype": ubx.FieldSpec{WireName: "public_iptype"},
		"PublicNetworkAccess": ubx.FieldSpec{WireName: "public_network_access"},
		"RestrictOutboundNetworkAccess": ubx.FieldSpec{WireName: "restrict_outbound_network_access"},
		"State": ubx.FieldSpec{WireName: "state"},
		"StateReason": ubx.FieldSpec{WireName: "state_reason"},
		"TrustedExternalTenants": ubx.FieldSpec{
			WireName: "trusted_external_tenants",
			Kind: "list",
			Fields: KustoKustoCluster_Properties_AcceptedAudiencesFields,
		},
		"Uri": ubx.FieldSpec{WireName: "uri"},
		"VirtualClusterGraduationProperties": ubx.FieldSpec{WireName: "virtual_cluster_graduation_properties"},
		"VirtualNetworkConfiguration": ubx.FieldSpec{
			WireName: "virtual_network_configuration",
			Kind: "object",
			Fields: KustoKustoCluster_Properties_VirtualNetworkConfigurationFields,
		},
		"ZoneStatus": ubx.FieldSpec{WireName: "zone_status"},
	}

var KustoKustoCluster_SkuFields = ubx.FieldMap{
		"Capacity": ubx.FieldSpec{WireName: "capacity"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"Tier": ubx.FieldSpec{WireName: "tier"},
	}

type KustoKustoClusterConfig struct {
	// "If etag is provided in the response body, it may also be provided as a header per the normal etag convention. Entity tags are used for comparing two or more entities from the same requested resource. HTTP/1.1 uses entity tags in the etag (section 14.19), If-Match (section 14.24), If-None-Match (section 14.26), and If-Range (section 14.27) header fields.")
	Etag any
	// Identity for the resource.
	Identity any
	// Class representing the Kusto cluster properties.
	Properties any
	// Azure SKU definition.
	Sku any
	// The availability zones.
	Zones any
}

type KustoKustoClusterAttrs struct {
	// "If etag is provided in the response body, it may also be provided as a header per the normal etag convention. Entity tags are used for comparing two or more entities from the same requested resource. HTTP/1.1 uses entity tags in the etag (section 14.19), If-Match (section 14.24), If-None-Match (section 14.26), and If-Range (section 14.27) header fields.")
	Etag any
	// Identity for the resource.
	Identity any
	// Class representing the Kusto cluster properties.
	Properties any
	// Azure SKU definition.
	Sku any
	// The availability zones.
	Zones any
}

var KustoKustoCluster = ubx.ResourceBinding{
	WireType: "azure_azure_kusto_kusto_cluster",
	Fields: ubx.FieldMap{
		"Etag": ubx.FieldSpec{WireName: "etag"},
		"Identity": ubx.FieldSpec{
			WireName: "identity",
			Kind: "object",
			Fields: KustoKustoCluster_IdentityFields,
		},
		"Properties": ubx.FieldSpec{
			WireName: "properties",
			Kind: "object",
			Fields: KustoKustoCluster_PropertiesFields,
		},
		"Sku": ubx.FieldSpec{
			WireName: "sku",
			Kind: "object",
			Fields: KustoKustoCluster_SkuFields,
		},
		"Zones": ubx.FieldSpec{WireName: "zones"},
	},
}
