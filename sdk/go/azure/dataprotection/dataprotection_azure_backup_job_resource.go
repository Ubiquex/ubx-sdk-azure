// Code generated by `ubx sdk gen` from azure_dataprotection_dataprotection_azure_backup_job_resource. DO NOT EDIT.
package dataprotection

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DataprotectionAzureBackupJobResource_Properties_ErrorDetails_Details struct {
}

type DataprotectionAzureBackupJobResource_Properties_ErrorDetails_InnerError struct {
	AdditionalInfo any
	Code any
	EmbeddedInnerError any
}

type DataprotectionAzureBackupJobResource_Properties_ErrorDetails struct {
	Code any
	Details any
	InnerError any
	IsRetryable any
	IsUserError any
	Message any
	Properties any
	RecommendedAction any
	Target any
}

type DataprotectionAzureBackupJobResource_Properties_ExtendedInfo_SourceRecoverPoint struct {
	RecoveryPointId any
	RecoveryPointTime any
}

type DataprotectionAzureBackupJobResource_Properties_ExtendedInfo_SubTasks struct {
	AdditionalDetails any
	TaskId any
	TaskName any
	TaskProgress any
	TaskStatus any
}

type DataprotectionAzureBackupJobResource_Properties_ExtendedInfo_WarningDetails struct {
	ResourceName any
	Warning any
}

type DataprotectionAzureBackupJobResource_Properties_ExtendedInfo struct {
	// Job's Additional Details
	AdditionalDetails any
	// State of the Backup Instance
	BackupInstanceState any
	// Number of bytes transferred
	DataTransferredInBytes any
	// Destination where restore is done
	RecoveryDestination any
	SourceRecoverPoint any
	// List of Sub Tasks of the job
	SubTasks any
	TargetRecoverPoint any
	// A List, detailing the warnings related to the job
	WarningDetails any
}

type DataprotectionAzureBackupJobResource_Properties struct {
	// Job Activity Id
	ActivityId any
	// Name of the Backup Instance
	BackupInstanceFriendlyName any
	// ARM ID of the Backup Instance
	BackupInstanceId any
	// ARM ID of the DataSource
	DataSourceId any
	// Location of the DataSource
	DataSourceLocation any
	// User Friendly Name of the DataSource
	DataSourceName any
	// Data Source Set Name of the DataSource
	DataSourceSetName any
	// Type of DataSource
	DataSourceType any
	DestinationDataStoreName any
	// Total run time of the job. ISO 8601 format.
	Duration any
	// EndTime of the job(in UTC)
	EndTime any
	// A List, detailing the errors related to the job
	ErrorDetails any
	Etag any
	// Extended Information about the job
	ExtendedInfo any
	// Indicated that whether the job is adhoc(true) or scheduled(false)
	IsUserTriggered any
	// It indicates the type of Job i.e. Backup:full/log/diff ;Restore:ALR/OLR; Tiering:Backup/Archive ; Management:ConfigureProtection/UnConfigure
	Operation any
	// It indicates the type of Job i.e. Backup/Restore/Tiering/Management
	OperationCategory any
	// ARM ID of the policy
	PolicyId any
	// Name of the policy
	PolicyName any
	// Indicated whether progress is enabled for the job
	ProgressEnabled any
	// Url which contains job's progress
	ProgressUrl any
	// Priority to be used for rehydration
	RehydrationPriority any
	// It indicates the sub type of operation i.e. in case of Restore it can be ALR/OLR
	RestoreType any
	SourceDataStoreName any
	// Resource Group Name of the Datasource
	SourceResourceGroup any
	// SubscriptionId corresponding to the DataSource
	SourceSubscriptionId any
	// StartTime of the job(in UTC)
	StartTime any
	// Status of the job like InProgress/Completed/Failed/Cancelled/CompletedWithWarnings/Cancelling/Paused
	Status any
	// Subscription Id of the corresponding backup vault
	SubscriptionId any
	// List of supported actions
	SupportedActions any
	// Name of the vault
	VaultName any
}

type DataprotectionAzureBackupJobResourceConfig struct {
	JobId any
	SourceBackupVaultId any
	SourceRegion any
	// path parameter, not part of the API's own resource representation
	Location any
}

type DataprotectionAzureBackupJobResourceAttrs struct {
	JobId any
	// AzureBackup Job Class
	Properties any
	SourceBackupVaultId any
	SourceRegion any
	// path parameter, not part of the API's own resource representation
	Location any
}

var DataprotectionAzureBackupJobResource = ubx.ResourceBinding{
	WireType: "azure_dataprotection_dataprotection_azure_backup_job_resource",
	Fields: ubx.FieldMap{
		"JobId": ubx.FieldSpec{WireName: "job_id"},
		"SourceBackupVaultId": ubx.FieldSpec{WireName: "source_backup_vault_id"},
		"SourceRegion": ubx.FieldSpec{WireName: "source_region"},
		"Location": ubx.FieldSpec{WireName: "location"},
	},
}
