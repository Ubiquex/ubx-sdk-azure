// Code generated by `ubx sdk gen` from azure_appplatform_appplatform_service_resource. DO NOT EDIT.
package appplatform

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type AppplatformServiceResource_Properties_MarketplaceResource struct {
	// The plan id of the 3rd Party Artifact that is being procured.
	Plan any
	// The 3rd Party artifact that is being procured.
	Product any
	// The publisher id of the 3rd Party Artifact that is being bought.
	Publisher any
}

type AppplatformServiceResource_Properties_NetworkProfile_IngressConfig struct {
	// Ingress read time out in seconds.
	ReadTimeoutInSeconds any
}

type AppplatformServiceResource_Properties_NetworkProfile_OutboundIps struct {
	// A list of public IP addresses.
	PublicIps any
}

type AppplatformServiceResource_Properties_NetworkProfile_RequiredTraffics struct {
	Direction any
	Fqdns any
	Ips any
	Port any
	Protocol any
}

type AppplatformServiceResource_Properties_NetworkProfile struct {
	// Name of the resource group containing network resources for customer apps in Azure Spring Apps
	AppNetworkResourceGroup any
	// Fully qualified resource Id of the subnet to host customer apps in Azure Spring Apps
	AppSubnetId any
	// Ingress configuration payload for Azure Spring Apps resource.
	IngressConfig any
	// Desired outbound IP resources for Azure Spring Apps resource.
	OutboundIps any
	// The egress traffic type of Azure Spring Apps VNet instances.
	OutboundType any
	// Required inbound or outbound traffics for Azure Spring Apps resource.
	RequiredTraffics any
	// Azure Spring Apps service reserved CIDR
	ServiceCidr any
	// Name of the resource group containing network resources of Azure Spring Apps Service Runtime
	ServiceRuntimeNetworkResourceGroup any
	// Fully qualified resource Id of the subnet to host Azure Spring Apps Service Runtime
	ServiceRuntimeSubnetId any
}

type AppplatformServiceResource_Properties_VnetAddons struct {
	// Indicates whether the data plane components(log stream, app connect, remote debugging) in vnet injection instance could be accessed from internet.
	DataPlanePublicEndpoint any
	// Indicates whether the log stream in vnet injection instance could be accessed from internet.
	LogStreamPublicEndpoint any
}

type AppplatformServiceResource_Properties struct {
	// Fully qualified dns name of the service instance
	Fqdn any
	// Purchasing 3rd Party product for one Azure Spring Apps instance
	MarketplaceResource any
	// Service network profile payload
	NetworkProfile any
	// Power state of the Service
	PowerState any
	// Provisioning state of the Service
	ProvisioningState any
	// ServiceInstanceEntity Id which uniquely identifies a created resource
	ServiceId any
	// Version of the Service
	Version any
	// Additional Service settings in vnet injection instance
	VnetAddons any
	ZoneRedundant any
}

type AppplatformServiceResource_Sku struct {
	// Current capacity of the target resource
	Capacity any
	// Name of the Sku
	Name any
	// Tier of the Sku
	Tier any
}

var AppplatformServiceResource_Properties_MarketplaceResourceFields = ubx.FieldMap{
		"Plan": ubx.FieldSpec{WireName: "plan"},
		"Product": ubx.FieldSpec{WireName: "product"},
		"Publisher": ubx.FieldSpec{WireName: "publisher"},
	}

var AppplatformServiceResource_Properties_NetworkProfile_IngressConfigFields = ubx.FieldMap{
		"ReadTimeoutInSeconds": ubx.FieldSpec{WireName: "read_timeout_in_seconds"},
	}

var AppplatformServiceResource_Properties_NetworkProfile_OutboundIpsFields = ubx.FieldMap{
		"PublicIps": ubx.FieldSpec{WireName: "public_ips"},
	}

var AppplatformServiceResource_Properties_NetworkProfile_RequiredTrafficsFields = ubx.FieldMap{
		"Direction": ubx.FieldSpec{WireName: "direction"},
		"Fqdns": ubx.FieldSpec{WireName: "fqdns"},
		"Ips": ubx.FieldSpec{WireName: "ips"},
		"Port": ubx.FieldSpec{WireName: "port"},
		"Protocol": ubx.FieldSpec{WireName: "protocol"},
	}

var AppplatformServiceResource_Properties_NetworkProfileFields = ubx.FieldMap{
		"AppNetworkResourceGroup": ubx.FieldSpec{WireName: "app_network_resource_group"},
		"AppSubnetId": ubx.FieldSpec{WireName: "app_subnet_id"},
		"IngressConfig": ubx.FieldSpec{
			WireName: "ingress_config",
			Kind: "object",
			Fields: AppplatformServiceResource_Properties_NetworkProfile_IngressConfigFields,
		},
		"OutboundIps": ubx.FieldSpec{
			WireName: "outbound_ips",
			Kind: "object",
			Fields: AppplatformServiceResource_Properties_NetworkProfile_OutboundIpsFields,
		},
		"OutboundType": ubx.FieldSpec{WireName: "outbound_type"},
		"RequiredTraffics": ubx.FieldSpec{
			WireName: "required_traffics",
			Kind: "list",
			Fields: AppplatformServiceResource_Properties_NetworkProfile_RequiredTrafficsFields,
		},
		"ServiceCidr": ubx.FieldSpec{WireName: "service_cidr"},
		"ServiceRuntimeNetworkResourceGroup": ubx.FieldSpec{WireName: "service_runtime_network_resource_group"},
		"ServiceRuntimeSubnetId": ubx.FieldSpec{WireName: "service_runtime_subnet_id"},
	}

var AppplatformServiceResource_Properties_VnetAddonsFields = ubx.FieldMap{
		"DataPlanePublicEndpoint": ubx.FieldSpec{WireName: "data_plane_public_endpoint"},
		"LogStreamPublicEndpoint": ubx.FieldSpec{WireName: "log_stream_public_endpoint"},
	}

var AppplatformServiceResource_PropertiesFields = ubx.FieldMap{
		"Fqdn": ubx.FieldSpec{WireName: "fqdn"},
		"MarketplaceResource": ubx.FieldSpec{
			WireName: "marketplace_resource",
			Kind: "object",
			Fields: AppplatformServiceResource_Properties_MarketplaceResourceFields,
		},
		"NetworkProfile": ubx.FieldSpec{
			WireName: "network_profile",
			Kind: "object",
			Fields: AppplatformServiceResource_Properties_NetworkProfileFields,
		},
		"PowerState": ubx.FieldSpec{WireName: "power_state"},
		"ProvisioningState": ubx.FieldSpec{WireName: "provisioning_state"},
		"ServiceId": ubx.FieldSpec{WireName: "service_id"},
		"Version": ubx.FieldSpec{WireName: "version"},
		"VnetAddons": ubx.FieldSpec{
			WireName: "vnet_addons",
			Kind: "object",
			Fields: AppplatformServiceResource_Properties_VnetAddonsFields,
		},
		"ZoneRedundant": ubx.FieldSpec{WireName: "zone_redundant"},
	}

var AppplatformServiceResource_SkuFields = ubx.FieldMap{
		"Capacity": ubx.FieldSpec{WireName: "capacity"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"Tier": ubx.FieldSpec{WireName: "tier"},
	}

type AppplatformServiceResourceConfig struct {
	// Service properties payload
	Properties any
	// Sku of Azure Spring Apps
	Sku any
}

type AppplatformServiceResourceAttrs struct {
	// Service properties payload
	Properties any
	// Sku of Azure Spring Apps
	Sku any
}

var AppplatformServiceResource = ubx.ResourceBinding{
	WireType: "azure_appplatform_appplatform_service_resource",
	Fields: ubx.FieldMap{
		"Properties": ubx.FieldSpec{
			WireName: "properties",
			Kind: "object",
			Fields: AppplatformServiceResource_PropertiesFields,
		},
		"Sku": ubx.FieldSpec{
			WireName: "sku",
			Kind: "object",
			Fields: AppplatformServiceResource_SkuFields,
		},
	},
}
